...
class CommissionReport(models.TransientModel):
    ...
    # def _onchange_to_date_validation(self):
    #     if self.start_date:
    #         if self.start_date > self.to_date and self.to_date:
    #             raise ValidationError(_('From date must be smaller than to date.'))
            ...
            for emp_id in self.employee_ids:
                if emp_id.id not in existing_emp:
                    ...
                    if commission_ex_emp:
                        ...
                        domain += [('user_id.employee_id', '=', emp_id.id)]
                        pos_order = self.env['pos.order'].sudo().search(domain)
                        if pos_order:
                            worksheet.write(row, 0, emp_id.name, header)
                            worksheet.write(row, 1, sum(pos_order.mapped('amount_total')), body)
                            worksheet.write(row, 2, sum(pos_order.mapped('amount_total')) * c_amount / 100, body)
                            ...
                            total_amount += sum(pos_order.mapped('amount_total'))
                            total_commission_amount += sum(pos_order.mapped('amount_total')) * c_amount / 100
                        else:
                            domain.pop()
        else:
                ...
                for commission_line in commission.pos_commission_ids:
                    ...
                    for emp in commission_line.employee_ids:
                        if emp.id not in existing_emp:
                            domain += [('user_id.employee_id', '=', emp.id)]
                            pos_order = self.env['pos.order'].sudo().search(domain)
                            if pos_order:
