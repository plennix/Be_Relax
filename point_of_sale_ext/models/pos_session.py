...
class PosSessionExt(models.Model):
    ...
    start_cash_register_currency_line_ids = fields.One2many('cash.register.currency.line', 'session_id',
                                                            string="Cash Register Currency Line Start")
    end_real_cash_register_currency_line_ids = fields.One2many('cash.register.currency.line', 'pos_session_id',
                                                               string="Cash Register Currency Line End")
    ...
    def get_payment_data(self, res_id):
        payments = {}
        payment_ids = self.env['pos.order'].search([('session_id', '=', res_id)]).mapped('payment_ids').filtered(
            lambda l: l.payment_method_id.journal_id.type == 'cash')
        for payment in payment_ids:
            is_same = False
            if payment.currency_id.name == self.env.company.currency_id.name:
                is_same = True
            if payment.pos_order_id.is_void_order:
                if payment.account_currency < 0:
                    if payment.currency_name not in payments.keys():
                        payments[payment.currency_name] = {'currency': payment.currency_name, 'account_currency': 0,
                                                           'amount': 0}
                    ...
                    payments[payment.currency_name].update(
                        {'account_currency': payment.account_currency + account_currency,
                         'amount': payment.amount + amount})
                else:
                    if payment.currency_id.name not in payments.keys():
                        payments[payment.currency_id.name] = {'currency': payment.currency_id.name,
                                                              'account_currency': 0}
                    amount = payments[payment.currency_id.name].get('account_currency', 0)
                    payments[payment.currency_id.name].update(
                        {'account_currency': payment.amount + amount, 'amount': payment.amount + amount})
            else:
                if payment.account_currency < 0:
                    if payment.currency_id.name not in payments.keys():
                        payments[payment.currency_id.name] = {'currency': payment.currency_id.name,
    ...
    def post_closing_cash_details(self, counted_cash, payments):
        ...
        self.write({
            'session_amt_diff_ids': [(0, 0, {'name': payment.get('name'), 'expected': payment.get('expected'),
                                             'counted': payment.get('counted'),
                                             'difference': payment.get('difference')}) for payment in payments]
        })
    ...
    def action_pos_session_open(self):
        ...
        for session in self.filtered(lambda session: session.state == 'opening_control'):
            ...
            if session.config_id.cash_control and not session.rescue:
                ...
                if not last_session:
                    session.write({
                        'start_cash_register_currency_line_ids': [
                            (0, 0, {'currency_name': currency.name, 'account_currency': 0}) for currency in poscurrency]
                    })
                else:
                    for currency in poscurrency:
                        start_cash_register_currency_line = session.start_cash_register_currency_line_ids.filtered(
                            lambda x: x.currency_name == currency.name)
                        end_real_cash_register_currency_line = last_session.end_real_cash_register_currency_line_ids.filtered(
                            lambda x: x.currency_name == currency.name)

                        if start_cash_register_currency_line and end_real_cash_register_currency_line:
                            start_cash_register_currency_line.write(
                                {'account_currency': end_real_cash_register_currency_line.account_currency})
                        else:
                            if end_real_cash_register_currency_line:
                                session.write({'start_cash_register_currency_line_ids': [(0, 0, {
    ...
    def get_closing_control_data(self):
        ...
        currency_cash_payments = orders.payment_ids.filtered(
            lambda
                p: p.payment_method_id == default_cash_payment_method_id and p.currency_name == self.env.company.currency_id.name or not p.currency_name)
        cash_lst.append({
    ...
    def convert_amount_to_currency(self, amount, currency_id):
        amount_to_currency = self.env.company.currency_id._convert(amount,
                                                                   self.env['res.currency'].browse(int(currency_id)),
                                                                   self.env.company, fields.Datetime.now())
    ...
    def get_sale_by_cashier(self, order_ids):
        ...
        for order in orders:
            ...
            for line in order.lines.filtered(
                    lambda l: l.product_id != l.order_id.session_id.config_id.tip_product_id and not l.is_reward_line):
                if line.employee_id.id in sale_by_staff.keys():
                    ...
                    sale_by_staff[line.employee_id.id].update({
                        ...
                        'amount_tax': round(
                            therapist_vals.get('amount_tax', 0) + line.price_subtotal_incl - line.price_subtotal, 2),
                ...
                if reward.id not in rewards_applied and reward.reward_type == 'discount':
                    ...
                    if reward.discount_applicability == 'order':
                        for sales in sale_by_staff:
                            ...
                            if sales in lines_employees:
                                ...
                                if reward.discount_mode == 'percent':
                                    amount_disc_with_tax = (reward.discount / 100) * therapist_disc_vals.get('amount',
                                                                                                             0)
                                    amount_disc_wo_tax = (reward.discount / 100) * therapist_disc_vals.get(
                                        'amount_without', 0)
                                    ...
                                    sale_by_staff[sales].update({
                                        ...
                                        'amount_without': round(
                                            therapist_disc_vals.get('amount_without', 0) - amount_disc_wo_tax, 2),
                                    })
                                else:
                                    lines_applicable = order.lines.filtered(lambda
                                                                                l: l.product_id != l.order_id.session_id.config_id.tip_product_id and not l.is_reward_line)
                                    fix_reward_amount = reward.discount / len(lines_applicable)
                                    sale_by_staff[sales].update({
                                        'amount': round(therapist_disc_vals.get('amount', 0) - fix_reward_amount, 2),
                                        'amount_without': round(
                                            therapist_disc_vals.get('amount_without', 0) - fix_reward_amount, 2),
                                    })
                    elif reward.discount_applicability == 'specific':
                        discountable_line = order.lines.filtered(
                            lambda l: l.product_id.id in reward.discount_product_ids.ids and l.product_id !=
                        ...
                        for discount_line in discountable_line:
                            ...
                            if reward.discount_mode == 'percent':
                                amount_discounted_with_tax = round(
                                    (reward.discount / 100) * discount_line.price_subtotal_incl, 2)
                                amount_discounted_wo_tax = round((reward.discount / 100) * discount_line.price_subtotal,
                                                                 2)
                                ...
                                if discount_line.employee_id.id in sale_by_staff.keys():
                                    ...
                                    sale_by_staff[discount_line.employee_id.id].update({
                                        'amount': round(
                                            therapist_reward_vals.get('amount', 0) - amount_discounted_with_tax, 2),
                                        'amount_tax': round(
                                            therapist_reward_vals.get('amount', 0) - amount_discounted_with_tax, 2) -
                                                      round(therapist_reward_vals.get('amount_without',
                                                                                      0) - amount_discounted_wo_tax, 2),
                                        'amount_without': round(
                                            therapist_reward_vals.get('amount_without', 0) - amount_discounted_wo_tax,
                                            2),
                                    })
                            else:
                                therapist_reward_vals = sale_by_staff[discount_line.employee_id.id]
                                sale_by_staff[discount_line.employee_id.id].update({
                                    ...
                                    'amount_without': round(
                                        therapist_reward_vals.get('amount_without', 0) - reward.discount, 2),
                                })
                            # used_lines.append(discount_line.id)

            therapists = order.lines.filtered(lambda
                                                  l: l.product_id != l.order_id.session_id.config_id.tip_product_id and not l.is_reward_line).mapped(
                'employee_id') | order.cashier_tip_ids.mapped('cashier_id')

            for therapist in therapists:
                tip_amount = sum(
                    order.cashier_tip_ids.filtered(lambda t: t.cashier_id.id == therapist.id).mapped('tip'))
                if therapist.id in sale_by_staff.keys():
                    thera_vals = sale_by_staff[therapist.id]
                    sale_by_staff[therapist.id].update(
                        {'tip_amount': round(thera_vals.get('tip_amount', 0) + tip_amount, 2)})
                else:
    ...
    def print_report_ext(self):
        data = {
            ...
            'discount_amount': sum(
                self.order_ids.mapped('lines').filtered(lambda x: x.is_reward_line).mapped('price_subtotal_incl')),
            'total_without_tax_aed': sum(self.order_ids.mapped('lines').mapped('price_subtotal')),
            # self.currency_id._convert(sum(self.order_ids.mapped('lines').mapped('price_subtotal')), currency_aed, self.company_id, datetime.now()),
            'sale_by_staff': self.env['pos.order.line'].read_group([('id', 'in', self.order_ids.mapped('lines').ids)],
                                                                   fields=['price_subtotal_incl',
                                                                           'tax_ids_after_fiscal_position',
                                                                           'price_subtotal'], groupby=['employee_id']),
            'payment_modes': self.env['pos.payment'].read_group([('session_id', '=', self.id)], fields=['pos_order_id'],
        ...
        return self.env.ref('point_of_sale_ext.pos_ord_session_reprt').with_context(
            summary_report_name=summary_report_name).report_action(docids=self, data=data)
    ...
    def get_print_report(self):
        data = {
            ...
            'sale_by_staff': self.env['pos.order.line'].read_group([('id', 'in', self.order_ids.mapped('lines').ids)],
                                                                   fields=['order_id'], groupby=['employee_id']),
...
class PosLine(models.Model):
    ...
    def get_order_line(self, orderlines):
        ...
        for product_id, olines in groupby(
                sorted(orderlines.filtered(lambda s: s.product_id != s.order_id.session_id.config_id.tip_product_id),
                       key=lambda l: l.product_id.id), key=lambda l: l.product_id.id):
        ...
        for s in lines_data.values():
            if s.get('order_lines', False):
                orderline_list.append({
                    ...
                    'tax_ids_after_fiscal_position': s.get('order_lines', False).mapped(
                        'tax_ids_after_fiscal_position'),
        ...
        for line in orderlines:
            if line.tax_ids_after_fiscal_position:
                line_taxes = line.tax_ids_after_fiscal_position.sudo().compute_all(
                    line.price_unit * (1 - (line.discount or 0.0) / 100.0), line.order_id.currency_id, line.qty,
